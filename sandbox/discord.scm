...
(define (discord-client? client)
    (pp (list "[discord-client?]" client (client 'get-interface) (client 'get-platform-id) (equal? (client 'get-platform-id) 'discord)))
    (equal? (client 'get-platform-id) 'discord))
; (define (discord? lst)
...
(define (make-discord! config)
    (pp (list "A discord client has been created with config:" config))
...
(define-generic-procedure-handler start-client!
    (match-args discord-client?)
    (lambda (client)
        (discord-i:start! (client 'get-interface))))

(define-generic-procedure-handler read-client!
    (match-args discord-client?)
    (lambda (client)
        (pp (list "Discord read" client))
        (discord-i:read! (client 'get-interface))))

(define-generic-procedure-handler write-client!
    (match-args discord-client? discord-chat-event?) ; TODO: rethink this
    (lambda (client event)
        (discord-i:write! (client 'get-interface) event)))
